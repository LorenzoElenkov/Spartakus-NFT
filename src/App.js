...
const App = () => {
  ...
  const trackScroll = (e) => {
    ...
    if (moveNext === true && isInSite) {
      ...
      if (currentPage === 1 && delta < 0) {
        ...
        if (
          e.deltaY % Math.floor(e.deltaY) !== 0 ||
          e.deltaY % 19 === 0 ||
          e.wheelDeltaY % 54 === 0 ||
          e.deltaY % 125 === 0 ||
          e.deltaY % 3 === 0
  ...
  return (
    <StyledApp>
      {!isInSite && (
        <Loading clicked={handleLoadingClick} images={imagesLoaded.current} />
      )}
      {window.outerWidth < 429 && (
        <HeaderMobile
          music={handleVolume}
          ...
          roadmap={roadmapRef}
          faq={faqRef}
          showRT={() => setShowRT(true)}
        />
      )}
      <Navigation
        currentPage={currentPage}
        onLinkClick={handleLinkClick}
        onRTClick={(state) => setShowRT(state)}
      />
